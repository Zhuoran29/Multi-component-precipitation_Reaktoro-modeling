...
#%% Original solution (ind waste)
def ind_waste(
        feed_props = {    
            "Na+"  : 304.5*1000*molar_mass["Na+"]/molar_mass["Halite"]+22.6*2*1000*molar_mass["Na+"]/molar_mass["Na2SO4"],
            "Cl-"  : 304.5*1000*molar_mass["Cl-"]/molar_mass["Halite"],
            "SO4-2" : 22.6*2*1000*molar_mass["SO4-2"]/molar_mass["Na2SO4"],
            "Mg+2" : 25.5,
            "Ca+2" : 465.3,
            "HCO3-": 142.5,
            },
...
ori_output, ori_state, ori_system, ori_props = ind_waste(RR=0)


#%% Test 1: Add CO2 at different pH
def decalcification(
        state, system,
        pressure = 1.01325, # bar
        add_CO2 = 0, #atm
    ...
    state.add("H2O"  ,  - remove_H2O / 1e6, "kg")
...
permian_brine_output, permian_brine_state, permian_brine_system, permian_brine_props = ind_waste(RR=0.5)
output_1_PB, props_1_PB, state_1_PB, system_1_PB = decalcification(state = permian_brine_state,
...
print('Ca rec rate', (float(props_1_PB.speciesAmount('Calcite'))) / float(props_1_PB.elementAmount('Ca')))
print('Mg rec rate', (float(props_1_PB.speciesAmount('Magnesite'))+float(props_1_PB.speciesAmount('Brucite'))) / float(props_1_PB.elementAmount('Mg')))

#%% Step 1: Plot (add CO2 at constant pH)
def plot_add_CO2(       
        CO2 = 1500, 
        runs = 50,
        pH=11,
    ...
    CO2s = [i * CO2 / runs for i in range(0,runs+1,1)]
    for v in CO2s:
        ori_output, ori_state, ori_system, ori_props = ind_waste(RR=0)

        output, props, state, system = decalcification(
            state=ori_state, 
            system=ori_system,
            pressure = 1.01325, # bar
            add_CO2= v, # L
        ...
        Ca_rr.append(float(props.speciesAmount('Calcite')) / float(props.elementAmount('Ca')))
        Mg_rr.append((float(props.speciesAmount('Brucite'))+float(props.speciesAmount('Magnesite'))) / float(props.elementAmount('Mg')))
    ...
    # ax2 = ax1.twinx()
    ax1.plot(pHs, Ca_rr, 'b-', label='Ca(OH)2')
    # ax1.plot(pHs, Brucites, 'r-', label='Mg(OH)2')
    # ax1.plot(pHs, Gypsums, 'g-', label='CaSO4')


    ax1.set_xlabel(f'CO2 (pH={pH})')
    ax1.set_ylabel('Ca_rr', color='k')
    ...
    return Brucites, Portlandites, Ca_rr, pHs, vols


# Run function

Brucites, Portlandites, Mg_rr, pHs, vols = plot_add_CO2(
                                                    CO2=15000,
                                                    runs = 30,
                                                    pH =9
                                                    )
