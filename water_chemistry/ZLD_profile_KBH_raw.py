...
ori_output, ori_state, ori_system, ori_props = permian_brine(RR=0)
#%% Step 0: Plot
def plot_step_0_permian(       
    ...
    ax2.plot(RRs, pHs, 'r-', label='CaSO4 2H2O')

    ax1.set_xlabel('Desal recovery rate')
    ax1.set_ylabel('Gypsum (mmol/L)', color='k')
...
permian_brine_output, permian_brine_state, permian_brine_system, permian_brine_props = permian_brine(RR=0)
...
def plot_step_1_permian(       
    ...
    ax2.plot(pHs, vols, 'g-', label='volume')
    ...
    print('Max mass: ', maxNaOH_mass, "Conc: ", NaOH_conc)
...
df = pd.DataFrame(results)
outpath = '/Users/zhuoranzhang/Documents/Crystallization_paper/ZLD_profile_results/'
filename = f'KBHraw_S1_NaOH{NaOH_conc}M.csv'
